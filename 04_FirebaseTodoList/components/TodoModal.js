...
export default class TodoModal extends React.Component {
    ...
    renderTodo = (todo, index) => {
        return (
            <Swipeable onRightAction={(_, dragX) => this.rightActions(dragX, index)}>
        ...
        return (
            <TouchableOpacity onPress={() => this.deleteTodo(index)}>
                <View style={[styles.deleteButton, { opacity: opacity }]}>
                    <Text style={{ color: colors.white, fontWeight: "800", transform: [{ scale }] }}>
                        Delete
                     </Text>
                </View>
            </TouchableOpacity>
    ...
    render() {
        ...
        return (
            <KeyboardAvoidingView style={{ flex: 1 }} behavior={(Platform.OS === 'ios') ? "padding" : null} enabled keyboardVerticalOffset={Platform.select({ ios: 80, android: 500 })}>
